...
export async function getAdminServices(params: GetAdminServicesParams = {}) {
  ...
  if (params.size_category_id)
    query.set("size_category_id", params.size_category_id);
  if (params.store_id) query.set("store_id", params.store_id);
  if (params.available_for_unlimited)
    query.set("available_for_unlimited", params.available_for_unlimited);
  if (params.service_location_type)
    query.set("service_location_type", params.service_location_type);
  const qs = query.toString();
  return apiAuthRequest<AdminServicesResponse>(
    `/services${qs ? `?${qs}` : ""}`,
  );
}
