      ...
      <CardContent className="flex flex-col gap-4 pt-6">
        ...
        {!previewLoading && previewData && (
          <div className="flex flex-col gap-5">
            ...
            {/* Promotion section disembunyikan untuk booking sisi customer
            <PromotionSection
              previewData={previewData}
...
function PreviewError({
  ...
  if (
    ...
    (error.toLowerCase().includes("zone") &&
      error.toLowerCase().includes("distance"))
  ) {
      ...
      <div className="flex items-start gap-3 rounded-xl border border-red-300 bg-red-50 px-4 py-3 dark:border-red-700 dark:bg-red-950/30">
        ...
        <div className="flex flex-col gap-1">
          <p className="text-sm font-semibold text-red-800 dark:text-red-300">
            Lokasi kamu di luar jangkauan layanan
          </p>
          <p className="text-xs text-red-700 dark:text-red-400">
            Lokasi kamu berada di luar radius zona{" "}
            {isHomeService ? "home service" : "pickup/delivery"} yang tersedia.
            {distKm && (
              <>
                {" "}
                Jarak ke store: <strong>{distKm} km</strong>.
              </>
            )}
          </p>
          <ul className="mt-0.5 flex flex-col gap-0.5 text-xs text-red-700 dark:text-red-400">
            {isHomeService && (
              <li>
                • Coba pilih layanan <strong>In Store</strong> sebagai
                alternatif.
              </li>
            )}
            <li>• Coba pilih store lain yang lebih dekat dengan lokasimu.</li>
            <li>
              • Pastikan alamat di profil kamu sudah sesuai dan koordinatnya
              akurat.
            </li>
            <li>
              • Jika masih bermasalah, hubungi store untuk informasi lebih
              lanjut.
            </li>
          </ul>
        </div>
  ...
  if (
    ...
    (error.toLowerCase().includes("location") ||
      ...
      error.toLowerCase().includes("longitude"))
  ) {
    return (
        ...
        <div className="flex flex-col gap-1">
          ...
          <p className="text-xs text-amber-700 dark:text-amber-400">
            ...
            {selectedLocationType === "in home"
              ? "home service"
              : "pickup/delivery"}{" "}
            membutuhkan koordinat lokasi pada profil akun kamu. Silakan lengkapi
            alamat di profil.
          </p>
        </div>
      </div>
  ...
  if (!previewData.pricing.has_active_membership) {
    return (
      <div className="flex items-center gap-2 rounded-xl border border-border/40 bg-muted/30 px-4 py-3 text-sm text-muted-foreground">
        ...
        <span>
          Anabul kamu belum memiliki membership aktif. Tidak ada benefit yang
          tersedia.
        </span>
      </div>
    );
    ...
    <div className="flex flex-col gap-2.5">
      <div className="flex items-center gap-2">
        ...
        <span className="rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-bold text-primary">
          {
            previewData.pricing.available_benefits.filter((b) => b.can_apply)
              .length
          }{" "}
          tersedia
        </span>
...
function PromotionSection({
  ...
  if ((previewData.pricing.available_promotions?.length ?? 0) === 0)
    return null;
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="overflow-hidden rounded-xl border border-border/50 bg-card">
        ...
        <FeeRow
          label="Biaya Pickup"
          fee={previewData.pricing_breakdown.pickup_fee}
          previewData={previewData}
          selectedBenefitIds={selectedBenefitIds}
        />
        <FeeRow
          label="Biaya Delivery"
          fee={previewData.pricing_breakdown.delivery_fee}
          previewData={previewData}
          selectedBenefitIds={selectedBenefitIds}
        />
        {selectedLocationType === "in home" && (
          <FeeRow
            ...
            fee={previewData.pricing_breakdown.travel_fee}
            previewData={previewData}
            selectedBenefitIds={selectedBenefitIds}
          />
        )}
        ...
        {(() => {
          ...
          const benefitDiscount =
            selectedBenefitIds.length > 0 && applyBenefitResult
              ? applyBenefitResult.total_discount
              : 0;
          const promoDiscount =
            selectedPromotionIds.length > 0 && applyPromotionResult
              ? applyPromotionResult.total_discount
              : 0;
          const displayTotal = grandTotal - benefitDiscount - promoDiscount;
          const showSkeleton =
            (selectedBenefitIds.length > 0 && applyBenefitLoading) ||
            (selectedPromotionIds.length > 0 && applyPromotionLoading);
          return (
...
function ServicePriceRow({
  ...
  const b = previewData.pricing.available_benefits.find(
    (x) =>
      ...
      (!x.service_id || x.service_id === selectedServiceId) &&
    ...
    <div className="flex items-center justify-between px-4 py-2.5 text-sm">
      ...
      {b ? (
        <PriceWithDiscount
          ...
          isQuota={!!isQuota}
          discountAmount={b.amount_discount ?? 0}
        />
      ) : (
  ...
  return (
    <div className="flex flex-col items-end gap-0.5">
      ...
      <span className="font-semibold text-primary">
        {isQuota
          ? "Gratis"
          : formatPrice(Math.max(0, original - discountAmount))}
      </span>
    </div>
  );
}
